...
-- Add exam category prompts (using separate INSERT statements to avoid conflicts)
INSERT INTO vocab_prompts (category, subcategory, prompt, difficulty_level) 
SELECT 'exam', 'gre', 'Generate academic/literary/test-prep level vocabulary for GRE preparation.
GUARDRAILS:
...
WHERE NOT EXISTS (
    SELECT 1 FROM vocab_prompts WHERE category = 'exam' AND subcategory = 'gre'
);

INSERT INTO vocab_prompts (category, subcategory, prompt, difficulty_level) 
SELECT 'exam', 'ielts', 'Generate academic vocabulary suitable for IELTS exam preparation with clear, precise meanings.
GUARDRAILS:
...
WHERE NOT EXISTS (
    SELECT 1 FROM vocab_prompts WHERE category = 'exam' AND subcategory = 'ielts'
);