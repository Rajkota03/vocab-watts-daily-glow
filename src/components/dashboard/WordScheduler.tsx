      ...
      duration: 0.3
        ...
        {settings.mode === 'auto' ? <div className="mb-6">
            ...
            <div className="space-y-2">
              {Array.from({
            length: wordCount
          }, (_, index) => <MotionDiv key={index} className="flex items-center gap-3 h-11 px-3 bg-gradient-to-r from-slate-50 to-slate-100 rounded-lg border border-slate-200 hover:border-slate-300 transition-colors" whileHover={{
            x: 2
          }} transition={{
            duration: 0.15
          }}>
                  ...
                  <Input type="time" value={settings.customTimes[index] || '09:00'} onChange={e => handleCustomTimeChange(index, e.target.value)} className="w-16 h-7 text-[11px] leading-4 border-slate-300 rounded-md bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500" min="06:00" max="23:00" />
                </MotionDiv>)}
            </div>
          </div>}
        ...
        <div className="border-t border-slate-100 pt-4">
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hidden">
            {previewTimes.map((time, index) => <MotionDiv key={index} className="flex-shrink-0 bg-white border border-slate-200 rounded-full px-2 py-1 text-[10px] leading-3 font-medium text-slate-600 whitespace-nowrap" whileHover={{
            scale: 1.05,
