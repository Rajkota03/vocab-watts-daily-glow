...
import { BookOpen, Briefcase, MessageSquare, GraduationCap, Smile, Sparkles, Heart, Zap, RefreshCw, Check, ListOrdered } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hidden">
        <div className="bg-white rounded-xl p-3 mb-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {categories.map(category => <button key={category.id} onClick={() => handlePrimarySelect(category.id)} className={cn("relative flex flex-col items-center justify-center p-3 rounded-xl transition-all duration-200", "bg-gradient-to-br shadow-sm", "hover:shadow-md", category.color, category.hoverColor, selectedPrimary === category.id && ["ring-2 ring-offset-2", category.activeColor], !isPro && "opacity-50 cursor-not-allowed")}>
                <div className="mb-2">{category.icon}</div>
                <span className="text-xs font-medium text-center leading-tight">
                  {category.name}
                </span>
                
                {selectedPrimary === category.id && <div className="absolute -top-1.5 -right-1.5 bg-white rounded-full shadow-sm p-0.5">
        ...
        <div className="bg-white rounded-xl p-3 mb-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(selectedPrimary === 'exam' ? examTypes : difficultyLevels).map(level => <button key={level.id} onClick={() => handleDifficultySelect(level.id)} className={cn("p-3 rounded-xl text-sm font-medium transition-all duration-200", "bg-gradient-to-br shadow-sm flex flex-col items-center text-center min-h-[80px] justify-center", level.color, "hover:shadow-md", selectedSubcategory === level.id && ["ring-2 ring-offset-2", level.activeColor])}>
