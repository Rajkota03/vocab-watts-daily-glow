      ...
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {isLoggedIn && isAdmin && (
            <Link 
              to="/admin" 
              className="text-glintup-text hover:text-glintup-mint transition-colors font-medium font-inter text-sm flex items-center gap-1"
            >
          ...
          {!isLoggedIn && (
            <div className="flex gap-3">
              <Button className="bg-white border border-glintup-indigo text-glintup-indigo hover:bg-glintup-indigo/10 shadow-sm" onClick={scrollToSignup}>
                Start Free Trial
              </Button>
              <Button className="bg-glintup-coral hover:bg-glintup-coral/90 text-white shadow-md" onClick={() => {
                scrollToSignup();
                setTimeout(() => {
          ...
          <div className="absolute top-full left-0 right-0 bg-white shadow-lg border-t border-gray-100 md:hidden animate-fade-in">
            <div className="flex flex-col p-4 space-y-3">
              ...
              {isLoggedIn && isAdmin && (
                <Link 
                  to="/admin" 
                  className="py-2 px-4 text-glintup-text hover:bg-gray-50 rounded-lg font-medium font-inter flex items-center gap-2" 
                  onClick={toggleMenu}
                >
                  <Settings className="h-4 w-4" />
                  Admin Dashboard
