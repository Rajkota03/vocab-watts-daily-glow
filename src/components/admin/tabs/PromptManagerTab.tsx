...
const PromptManagerTab: React.FC = () => {
  ...
  const fetchPrompts = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from("vocab_prompts").select("*").order("updated_at", {
      ascending: false
    });
    if (error) {
      toast({
  ...
  const handleSubmit = async (data: PromptForm) => {
    try {
      if (editingPrompt) {
        ...
        const {
          error
        } = await supabase.from("vocab_prompts").update({
        ...
        const {
          error
        } = await supabase.from("vocab_prompts").insert({
          category: data.category,
          subcategory: data.subcategory,
          prompt: data.prompt,
          difficulty_level: data.difficulty_level
        ...
        <CardContent>
          <div className="flex justify-between mb-4">
            ...
            <Dialog open={openDialog} onOpenChange={setOpenDialog}>
              ...
              <DialogContent className="sm:max-w-[520px] bg-zinc-50">
                <DialogHeader>
                ...
                <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
                  <div>
                    ...
                    required: true
                  })} placeholder="business, exam, slang, etc." />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    required: true
                  })} placeholder="OpenAI system/user prompt for this category/subcategory" />
                  </div>
                  <div>
                    <label className="font-medium mb-1">Difficulty Level</label>
                    <Input {...form.register("difficulty_level", {
                    required: true
                  })} placeholder="e.g. beginner, intermediate, professional, gre, etc." />
                  </div>
          ...
          {loading ? <div>Loading prompts...</div> : <div className="border rounded-md overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {prompts.map(p => <TableRow key={p.id}>
